...
< <?php //include_once 'resource/session.php'; ?>    
    ...
    <?php
    if ( isset($_POST['login_sbt'], $_POST['token']) ) 
    { 
        if ( validate_token($_POST['token']) )
        {
            ...
            { 	
                ...
                try{ 
                                     ...
                                     $sqlQuery = "SELECT * from tb_member where mem_name =:username";
                                     $statement= $db->query($sqlQuery);
                                      $statement->execute( array(':username'=> $user ) );
   

                                     if($row = $statement->fetch() ) 
                                     { # $statement->fetch()  --> fetches the data from the databse as an associative array , so 								$row is an associative array with the keys as the column names in the database, and values as the data in the data stored in the database..
                                        
                                        $id = $row['mem_id'];		
                                        echo '$id';				# storing the data from the databases  for further checking 
                                        $hashed_password = $row['mem_password'];	# for further verification and processing
                    ...
                    $result = "Something went wrong while searching for the user in database!";
